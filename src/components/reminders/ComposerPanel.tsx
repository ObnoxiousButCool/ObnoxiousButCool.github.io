...
export function ComposerPanel({
  ...
  return (
    <div className="rounded-2xl bg-white p-5 card-shadow">
      ...
      <Tabs defaultValue="composer" className="mt-5">
        <TabsList className="h-auto rounded-full bg-[#F5F3FF] p-1">
          <TabsTrigger value="composer" className="rounded-full px-4 py-2 text-sm data-active:bg-white">
            AI Composer
          </TabsTrigger>
          <TabsTrigger value="feed" className="rounded-full px-4 py-2 text-sm data-active:bg-white">
            Track Feed
          </TabsTrigger>
        </TabsList>
            ...
            {channels.map((option) => (
              <button
                ...
                className={cn(
                  ...
                  channel === option ? "bg-[#7C3AED] text-white" : "bg-[#F8FAFC] text-[#6B7280]"
                )}
              >
                {option}
                {option === "WhatsApp" ? <span className="ml-2 text-[11px] opacity-80">(Simulated)</span> : null}
              </button>
            ))}
          </div>

          <p className="mt-4 text-[11px] font-medium text-[#9CA3AF]">AI draft - edit as needed</p>

          <Textarea
            value={message}
            onChange={(event) => setMessage(event.target.value)}
            className="mt-3 min-h-[300px] rounded-2xl border-[#E5E7EB] bg-[#F8FAFC] p-4 text-sm leading-6 shadow-none"
          />
